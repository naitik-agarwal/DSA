...
class SegmentTree {
  ...
  int rangeMin(int qi, int qj, int si, int sj, int node) {
    ...
    // case 2
      ...
      int mid = si + (sj - si) / 2;
      return min(rangeMin(qi, qj, si, mid, 2 * node + 1),
                 rangeMin(qi, qj, mid + 1, sj, 2 * node + 2));
    }
  }
 ...
 public:
  ...
  int rangeMinquery(int qi, int qj) { return rangeMin(qi, qj, 0, n - 1, 0); }
...
signed main() {
  ...
  while (q--) {
    ...
    if (f == 2) {
      int a, b;
      cin >> a >> b;
      cout << st.rangeMinquery(a - 1, b - 1) << endl;
    } else {
      int k, u;
      cin >> k >> u;
      st.updateQuery(k - 1, u);
    }
  }
  return 0;
}